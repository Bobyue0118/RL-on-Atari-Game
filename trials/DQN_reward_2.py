...
class DQN:
    ...
    def replay(self):
        ...
        mini_batch = random.sample(self.memory,
                                   self.batch_size)  # 试了下，就是随机选取的 (cur_state, action, reward, new_state, done)
...
def main():
    ...
    for episode in range(episodes):
        ...
        reward_cnt += 1
        if reward_cnt % 100 == 0:
            print('episode:', episode, np.mean(graph_reward[reward_cnt-100:reward_cnt]))
        for i in range(trial_len):
            ...
            if i < 38:
                ...
                reward_store = np.sum(reward_[30:38])

                dqn_agent.remember(copy.deepcopy(prev_state_[0]), action_[0], reward_store, copy.deepcopy(current_state_[0]),
                                   done_[0])
